...
import { 
  ...
  Home, Settings, Sliders, PanelRight, Grid, LayoutGrid,
  Check
} from "lucide-react";
...
export default function DashboardPage() {
  ...
  // Defaultní widgety podle role
  const defaultWidgets = user?.role === "company" || user?.role === "admin" 
  ...
  // Iniciujeme state s výchozími hodnotami, aby nebyl nikdy null
  const [activeWidgets, setActiveWidgets] = useState<WidgetType[]>(defaultWidgets);
  ...
  // Inicializace layoutu pro výchozí widgety hned při první renderování
  useEffect(() => {
    if (gridLayout.length === 0 && activeWidgets.length > 0) {
      const initialLayout = generateDefaultLayout(activeWidgets);
      setGridLayout(initialLayout);
    }
  }, [activeWidgets, gridLayout.length, generateDefaultLayout]);
        ...
        return (
          <Card className="h-full">
            <CardHeader className="flex flex-row items-center justify-between pb-2 drag-handle">
              ...
              {/* Tlačítko pro odstranění widgetu - viditelné pouze v edit módu nebo na mobilu */}
              {(isEditMode || isMobile) && (
                <Button 
                  variant="ghost" 
                  size="icon" 
                  className="h-8 w-8 rounded-full" 
                  onClick={(e) => handleRemoveWidget(e, WidgetType.STATS)}
                >
                  <X className="h-4 w-4" />
                </Button>
              )}
            ...
            <CardHeader className="flex flex-row items-center justify-between pb-2 drag-handle">
              ...
              {(isEditMode || isMobile) && (
                <Button 
                  ...
                  size="icon" 
                  className="h-8 w-8 rounded-full" 
                  onClick={(e) => handleRemoveWidget(e, WidgetType.UPCOMING_SHIFTS)}
                >
                  <X className="h-4 w-4" />
                </Button>
              )}
  ...
  return (
    <Layout title="Dashboard">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
        <div>
          <div className="flex items-center">
            ...
            {/* Nové tlačítko pro úpravu dashboardu pro desktopová zařízení */}
            {!isMobile && (
              <Button 
                variant="ghost" 
                size="sm" 
        ...
        <div className="mt-4 md:mt-0 flex space-x-3">
          {/* Tlačítko na přidání widgetů - zobrazí se pouze v edit módu nebo na mobilu */}
          {(isEditMode || isMobile) && (
      ...
      {/* Informační banner v edit módu */}
      {!isMobile && isEditMode && (
        <div className="mb-4 p-2 bg-blue-50 text-blue-700 rounded-md flex items-center justify-center">
          <LayoutGrid className="h-4 w-4 mr-2" />
          <span className="text-sm">Režim úprav: Nyní můžete přetahovat a měnit velikost widgetů na dashboardu.</span>
        </div>
      )}
      
      {/* Grid Layout pro widgety */}
      <div className="mb-8">
        {gridLayout.length > 0 && activeWidgets.length > 0 ? (
          <ResponsiveReactGridLayout
            ...
            rowHeight={50}
            // V desktop módu je možnost přetahovat a měnit velikost widgetů pouze v edit módu
            // V mobilním módu (isMobile) je to povoleno vždy
            isDraggable={isMobile || isEditMode}
            isResizable={isMobile || isEditMode}
            onLayoutChange={handleLayoutChange}
            draggableHandle=".drag-handle" // Nastavení třídy pro drag rukojeť (headery karet)
